...
VoteTracker+ can enhance the voter vote-by-mail experience depending on the level of adoption by both election officials and the voter.  If configured by election officials, the voter can receive their anonymized ballot check by mail.  In this scenario election officials would place the absentee ballot (post voter identification validation etc) in a VTP scanner.  The VTP scanner would only print the anonymized ballot check and no ballot index would be display/printed.  The election official would then place the check in a return envelope to the voter.

However, if election officials have configured support for voter supplied mail-in public keys, if the voter has supplied their mail-in public key along with their ballot, the voter's ballot index into their check is encrypted with the public key and stored in a separate VTP private election repo.  This repo is not publicly available but is still part of the VoteTracker+ repo system.

After all the polls close and election officials have setup the required restricted access to the private repo, voters can visit election officials in person, nominally at their town hall or some other officially designated and secured location.  After the voter has properly identified themselves nominally in a manner similar to casting a ballot in person, an election official can privately and securely reveal to the voter their specific encrypted ballot index offset.  The voter can then enter the encrypted offset into the VTP mobile app or secured web portal to reveal their true check index.  With that index they can validate that their ballot has been cast, recorded, and tallied as intended via the public VoteTracker+ election repos.
