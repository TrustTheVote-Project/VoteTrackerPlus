...
On election day or whenever a voter casts her/his vote, the voter privately receives two digests - one for their ballot and one for their voter id.  Election officials on the other hand only receive a voter id digest.  Note that though both repos are public and hence distributed in an open source sense, the algorithms and cryptography being employed prevents the public from associating a voter id digest with a ballot digest.

When mail-in ballots are counted, either via UVBM, early, or absentee scenarios, election officials can/may/will provide both digests to the voter.

When a person votes in person with a physical ballot, they can receive either a physical or electronic copy of both digests.  Election official can choose how they wish to process ballots, with a few options being:

* the physical ballot can be printed with the ballot digest
* the voter id repo can be the sole record of the voter id digest
* the voter id digest can be printed and physically associated with the coter id roles

This allows local election officials to choose between fully electronic, partially electronic, or fully physical ballot record keeping.  As the election officials may need to recount votes or provide evidence of voter id, election officials can choose how they wish to comply with such requirements.  However, regardless of their choice VOTES will manage both the ballot and voter id repos for the public at large.
